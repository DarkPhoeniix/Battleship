...
Player::Player(Player &&obj) noexcept
    : _name(std::move(obj._name)), _playerMap(std::move(obj._playerMap)),
      _score(obj._score) {}
...
bool Player::setCoordinateState(const Coordinate &Coordinate,
                                TileState tileState) {
...
bool Player::setShips() {
  ...
  for (int i = 0; i < Map::MAP_SIZE; ++i) {
    for (int j = 0; j < Map::MAP_SIZE; j++) {
      if ((_playerMap.getTile(Coordinate(i, j)) & TileState::ShipAfloat) !=
          TileState::Empty) {
        shipCounter++;
      }
    }
...
void Player::addScore(unsigned score) { _score += score; }

std::string Player::getName() const { return _name; }

const Map &Player::getPlayerMap() const { return _playerMap; }

unsigned Player::getScore() const { return _score; }
} // namespace Battleship
