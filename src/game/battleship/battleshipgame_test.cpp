...
private slots:
    ...
    void testMapper() {
        Battleship::BattleshipGame game({{"Player_1"}, {"Player_2"}});

        for (const auto& item : correctCoords) {
            QCOMPARE(game.addTile(item.first, item.second, 0), Battleship::MappingStatus::Ok);
            QCOMPARE(game.addTile(item.first, item.second, 1), Battleship::MappingStatus::Ok);
        }
    ...
    void testMapScanner() {
        ...
        QCOMPARE(game.getTileState(5,5,0), Battleship::TileState::Empty | Battleship::TileState::WasShot);
        QCOMPARE(game.getTileState(0,0,1), Battleship::TileState::Empty | Battleship::TileState::WasShot | Battleship::TileState::ShipAfloat | Battleship::TileState::ShipSunk);
        QCOMPARE(game.getTileState(2,5,0), Battleship::TileState::Empty);
        QCOMPARE(game.getTileState(4,7,1), Battleship::TileState::Empty | Battleship::TileState::WasShot | Battleship::TileState::ShipAfloat);
    }
    ...
    void testTerminationChecker() {
        ...
        for (const auto& item : correctCoords) {
            QCOMPARE(game.registerTurn(item.first, item.second, 0), Battleship::TurnStatus::Ok);
        }
