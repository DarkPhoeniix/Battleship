...
namespace Battleship {
    ...
    bool Map::_validateFleetParameters() const {
        ...
        if (sizesOfShips != std::vector<uint8_t>{_SUBMARINE_COUNT,
                                                 _DESTROYER_COUNT,
                                                 _CRUISER_COUNT,
                                                 _BATTLESHIP_COUNT}) {
            result = false;
    ...
    bool Map::_isCoordinateInShip(const Ship& ship, const Coordinate& coordinate) const {
        bool result = false;
        auto shipCoords = ship.getCoordinates();

        result = true;
        if ((_map.at(coordinate) & TileState::ShipAfloat) == TileState::ShipAfloat &&
            find(shipCoords.begin(), shipCoords.end(), coordinate) == shipCoords.end())
        {
            result = false;
        }
    ...
    {
        _ships.reserve(_BATTLESHIP_COUNT +
                       _CRUISER_COUNT +
                       _DESTROYER_COUNT +
                       _SUBMARINE_COUNT);
