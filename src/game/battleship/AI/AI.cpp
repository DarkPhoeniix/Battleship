...
std::vector<Coordinate> AI::_placeShip(std::vector<Coordinate> &freeCoordinates,
  ...
  while (!flag) {
    badCoords.push_back(*freeCoordinates.erase(std::find(
        freeCoordinates.begin(), freeCoordinates.end(), generatedCoord)));
    generatedCoord = freeCoordinates[rand() % (freeCoordinates.size() - 1)];
    ...
    freeCoordinates.erase(std::lower_bound(
        freeCoordinates.begin(), freeCoordinates.end(), generatedCoord));
  }
...
AI::_tryToPlaceShip(const std::vector<Coordinate> &availableCoords,
                    Coordinate startCoord, int sizeOfShip) const {
  std::vector<Coordinate> coords = {startCoord};
  auto currentCoord = startCoord;

  auto tryToPlaceShipCoordinate = [this, &coords, &availableCoords,
                                   sizeOfShip](const Coordinate &coordinate) {
  ...
  for (int i = 0;
       i < coords.size() && coords.size() < sizeOfShip && i < sizeOfShip; ++i) {
...
bool AI::_isCoordAvailableForPlacement(
    const std::vector<Coordinate> &coordinates, Coordinate coordinate) const {
  return std::binary_search(coordinates.begin(), coordinates.end(), coordinate);
}
...
bool AI::_checkIsHuntingPhase(TileState prevTileStatus) {
  if (prevTileStatus ==
      (TileState::WasShot | TileState::ShipAfloat | TileState::ShipSunk)) {
    _isHuntingPhase = false;
  } else if (prevTileStatus == (TileState::WasShot | TileState::ShipAfloat)) {
...
Coordinate AI::_generateRandomAttack() {
  qDebug().verbosity(QDebug::MaximumVerbosity)
      << "AI generates turn in mode: random";
...
Coordinate AI::_generateDestroyingAttack() {
  qDebug().verbosity(QDebug::MaximumVerbosity)
      << "AI generates turn in mode: hunt";
  ...
  auto searcher = [&container = _availableCoords](Coordinate coordinate) {
    return std::find(container.begin(), container.end(), coordinate);
  };
  ...
  std::vector<std::pair<Coordinate, TileState>>::iterator lastHitIter =
      std::find_if(_prevShots.begin(), _prevShots.end(), isHitShip);
  while (attackCoordinate == _availableCoords.end()) {
    std::vector<uint8_t> availableDirection = {1, 2, 3, 4};
    while (!availableDirection.empty() &&
           attackCoordinate == _availableCoords.end()) {
      uint8_t randomizer = rand() % availableDirection.size();
      switch (availableDirection[randomizer]) {
      case 1:
        attackCoordinate = searcher(
            Coordinate(lastHitIter->first.x + 1, lastHitIter->first.y));
        break;
      case 2:
        attackCoordinate = searcher(
            Coordinate(lastHitIter->first.x - 1, lastHitIter->first.y));
      ...
      case 3:
        attackCoordinate = searcher(
            Coordinate(lastHitIter->first.x, lastHitIter->first.y + 1));
        break;
      case 4:
        attackCoordinate = searcher(
            Coordinate(lastHitIter->first.x, lastHitIter->first.y - 1));
    ...
    lastHitIter =
        std::find_if(std::next(lastHitIter), _prevShots.end(), isHitShip);
  }

  return *attackCoordinate;
}

AI::AI() : _isHuntingPhase(false) {
...
void AI::approveAttack(Coordinate coordinate, TileState tileStatus) {
  ...
  auto toErase =
      std::find(_availableCoords.begin(), _availableCoords.end(), coordinate);
...
Coordinate AI::generateAttack(TileState prevTileStatus) noexcept {
  ...
  try {
    if (!_prevShots.empty()) {
      _prevShots.rbegin()->second |=
          (prevTileStatus & (TileState::WasShot | TileState::ShipAfloat));
    }

    if (!_isHuntingPhase) {
      coord = std::find(_availableCoords.begin(), _availableCoords.end(),
