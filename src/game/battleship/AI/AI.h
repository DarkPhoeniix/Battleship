...
class AI {
private:
  std::vector<Coordinate> _placeShip(std::vector<Coordinate> &freeCoordinates,
  ...
  _tryToPlaceShip(const std::vector<Coordinate> &availableCoords,
                  Coordinate startCoord, int sizeOfShip) const;
  bool _isCoordAvailableForPlacement(const std::vector<Coordinate> &coordinates,
                                     Coordinate coordinate) const;
  void _removeAdjacentCoordinates(std::vector<Coordinate> &container,
...
public:
  ...
  Coordinate
  generateAttack(TileState prevTileStatus = TileState::Empty) noexcept;
